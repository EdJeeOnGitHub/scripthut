...
class RunManager:
    ...
    async def create_keyed_adhoc_run(self, key: str, payload: dict) -> Run | None:
        ...
        with journal.lease(key):
            ...
            return await self._build_run([task], payload.get('run_name') or f"_adhoc/{task.id}",
                ...
                reserved_run_id=record['run_id'], request_key=key, artifact_refs=payload.get('artifact_refs'))

    async def attach_artifact_outputs(self, run_id: str, value: dict) -> dict:
        ...
        async with self.submissions.lock(run):
            ...
            merged = snapshot(dict(existing, outputs=[outputs[name] for name in sorted(outputs)]))
    ...
    async def rerun_in_place(self, run_id: str) -> Run:
        ...
        if run.artifact_refs is not None:
            raise ValueError('Artifact-linked runs require a new launcher request; retry output collection separately')
